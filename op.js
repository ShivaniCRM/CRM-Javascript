    ...
    if(formContext.ui.tabs.get(tabName).sections.get(sectionName) != null)
     {
       ...
       var OpportunityType = formContext.getAttribute("new_opportunitytype").getValue();
       if (OpportunityType == 100000000 || 100000002 || 100000003) {
       	var CA = formContext.ui.tabs.get("Remarks").sections.get("tab_8_section_2"); 
       CA.setVisible(true);
       }
       else{
       var CA = formContext.ui.tabs.get("Remarks").sections.get("tab_8_section_2"); 
     ...
     if (AgreementDuration == null || SLAReponseTime1 == null || SLAResolutiontime1 == null || SLAReponseTime2 == null || SLAResolutiontime2 == null || SLAReponseTime3 == null || SLAResolutiontime3 == null)
      {
        var alertStrings = { text: "Please Enter All Agreement and SLA Details" };
					      ...
					      //added by swapnil on 8/11/2019
							    ...
							    function (success) {    
								    if (success.confirmed)
						  		   {	
                         formContext.getAttribute("new_opportunitysendforapproval").setValue(true); 				    
          ...
          else if (OpportunityType == "100000001") {
				...
				function success(result){
					if (result.name!=null) {
						if (result.name == "SR" || result.name == "RM") {
							...
							var confirmStrings =  {text : "Do you really wish to continue?", title : "Send for Approval"};
							var confirmOptions = { height: 200, width: 500};
							Xrm.Navigation.openConfirmDialog(confirmStrings, confirmOptions).then(
								function (success){
									if (success.confirmed)
					           	...
					           	var formContext = executionContext.getFormContext();
					           	 //added by swapnil on 8/11/2019
										  ...
										  Xrm.Navigation.openConfirmDialog(confirmStrings, confirmOptions).then( 
										    ...
										    if (success.confirmed)
										     {					    
formContext.getAttribute("new_opportunitysendforapproval").setValue(true);														formContext.getAttribute("new_sendforapproval").setValue(true);
					                 	TabEnableDisable(executionContext); //added by Swapnil on 8/11/2019
		 												formContext.data.entity.save();
										      }
										    else 
...
var isAsyncOperationComplted = false; 
...
debugger;
var Approved_flag = formContext.getAttribute("new_opportunityapproved").getValue();
var DisApproved_flag = formContext.getAttribute("new_opportunitydisapproved").getValue();
if (Approved_flag == true || DisApproved_flag == true) {
	return false;
}
else{


if (isAsyncOperationComplted) {
...
function ShowHideSFA(formContext){
        debugger;
	var SFA_Flag = formContext.getAttribute("new_opportunitysendforapproval").getValue();
	if (SFA_Flag) {
		return false;
	}
	else{
